---
name: xxd-panel-197
description: "Create Panel 197 raster artwork: Folk-art World Inside a Cage. Use when the user invokes xxd-panel-197 or requests this specific visual language."
---
...
The source mentions a reference illustration that is not bundled. Use its complete written folk-art and cage-world instructions; never claim to have seen that missing illustration. If the user requires matching that particular image, obtain it before generation.
...
Write final PNGs directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-197/` or the explicit output root. Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-197-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
